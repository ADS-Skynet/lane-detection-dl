...
class SyntheticDataGenerator:
    ...
    def augment_noise(self, img, noise_level=None):
    ...
    def augment_exposure(self, img, gamma_range=(0.5, 2.0)):
        """Simulate over/under exposure using gamma correction"""
    ...
    def augment_motion_blur_vertical(self, img, kernel_size=None):
        """Apply vertical motion blur"""
        if kernel_size is None:
            kernel_size = np.random.choice([5, 7, 9, 11, 13])
    ...
    def generate_augmentations(self, img, lanes, num_augmentations=5):
        ...
        for i in range(num_augmentations):
            ...
            # 2. Exposure (40% chance)
            if np.random.rand() > 0.6:
                aug_img = self.augment_exposure(aug_img)

            # 3. Hue/Saturation (60% chance)
            ...
            # 5. Vertical Motion Blur (30% chance)
            if np.random.rand() > 0.7:
                aug_img = self.augment_motion_blur_vertical(aug_img)
...
def main():
    # Configuration
    source_img_dir = '../dataset/original/images'
    source_ann_dir = '../dataset/original/annotations'
    output_img_dir = '../dataset/original_augmented/images'
    output_ann_dir = '../dataset/original_augmented/annotations'

    # Number of augmented versions per original image
    augmentations_per_image = 1  # Will create 10x dataset (1 original + 9 augmented)
